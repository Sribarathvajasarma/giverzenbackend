...
const userCtrl = {
  searchUser: async (req, res) => {
    try {
      await db.query(
        "select id,username,avatar from user where username LIKE '%" +
          req.query.username +
          "%'",
        (err, results) => {
          if (err) {
  ...
  addCommiteeMember: async (req, res) => {
    try {
      const { fullname, username, email, password, gender, mobile, address } =
        req.body;
              ...
              (err2, results2) => {
                ...
                if (results2.length !== 0) {
                  ...
                  if (password.length < 6)
                    ...
                    db.query(
                      "INSERT INTO user (fullname, username, email, password, gender, role, mobile, address, mongo_id) VALUES ('" +
                        fullname +
                        "','" +
                        username +
                        "', '" +
